...
for dataset_name in dataset_names:
    ...
    for i in range(max_trials):
        ...
        for j in range(num_guesses_ms):
            ...
            model = EuSN(num_units, epsilon = epsilon, gamma = gamma, 
    ...
    for i in range(num_guesses):
      initial_time = time()
      model = EuSN(best_num_units,epsilon = best_epsilon, gamma = best_gamma,input_scaling = best_input_scaling,
    ...
    for i in range(max_trials):
        ...
        val_score = 0
        for j in range(num_guesses_ms):
            model = ESN(num_units, leaky = leaky,
                      input_scaling = input_scaling, bias_scaling = bias_scaling,
    ...
    for i in range(num_guesses):
      initial_time = time()
      model = ESN(best_num_units,leaky = best_leaky,input_scaling = best_input_scaling,
    ...
    for i in range(max_trials):
        ...
        val_score = 0
        for j in range(num_guesses_ms):
            model = RESN(num_units, leaky = leaky,
                      input_scaling = input_scaling, bias_scaling = bias_scaling,
                      spectral_radius= spectral_radius)
            model.fit(x_train,y_train)
            val_score = val_score + model.evaluate(x_val,y_val)
    ...
    for i in range(num_guesses):
      initial_time = time()
      model = RESN(best_num_units,leaky = best_leaky,input_scaling = best_input_scaling,
                     bias_scaling = best_bias_scaling,spectral_radius =best_spectral_radius)
      model.fit(x_train, y_train)
      acc = model.evaluate(x_test,y_test)
      required_time.append(time()-initial_time)
    ...
    for i in range(max_trials):
        ...
        val_score = 0
        for j in range(num_guesses_ms):
            model = DeepESN(num_units, num_layers = num_layers,
                      leaky = leaky,
                      input_scaling = input_scaling, bias_scaling = bias_scaling,
                      spectral_radius= spectral_radius)
            model.fit(x_train,y_train)
    ...
    for i in range(num_guesses):
      initial_time = time()
      model = DeepESN(best_num_units,num_layers = best_num_layers,
    ...
    results_logger.write('Accuracy: MEAN {} STD {}\n'.format(np.mean(acc_ts),np.std(acc_ts)))
    results_logger.write('Model selection time: {} seconds = {} minutes\n'.format(elapsed_model_selection_time, elapsed_model_selection_time/60))
    results_logger.write('Average time for TR,TS: MEAN {} STD {}; in mins = {} ; {}\n'.format(np.mean(required_time),np.std(required_time),np.mean(required_time)/60,np.std(required_time)/60))
    results_logger.write('Model summary:\n')
    results_logger.write('trainable parameters = {}\n'.format(model.readout.coef_.shape[0] * model.readout.coef_.shape[1] + model.readout.intercept_.shape[0]))
